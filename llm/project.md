...
## Key Features
- **Breathing Animation**: A linearly animated 4-section lung-volume bar that visually represents the breathing phases. High contrast text labels ("In", "Out", "Hold") indicate the current phase. Updates every 100ms.
- **Vibration Modes**: Configurable haptic feedback to guide breathing without looking at the screen. The user's selection is saved across sessions.
  ...
  - **Off**: No vibrations. The watch backlight stays permanently on in this mode.
- **Controls**:
  - **Top Button**: Pause/Resume the breathing cycle, animation, and timer.
  - **Middle Button**: Cycle through the vibration modes.
  - **Bottom Button**: Reset the session timer and cycle phase.
- **UI Design**: A minimal, high-contrast layout driven by a vertical layout manager. Includes a 24-hour clock, session duration timer, and native vector-drawn state indicators for vibration mode and pause state. Supports color (Pebble Time series) and monochrome (Pebble Classic, Pebble 2) displays via fallback colors.
